    ...
    public RandomNumber(int minRange, int maxRange, int attempts) {
        ...
        while (attempt >= 1) {
            ...
            if (attempt >= 5) {
                ...
                System.out.println(String.format("Не угадал! Осталось %s попытки! Попробуй ещё раз!", --attempt));
            } else if (attempt == 0) {
                ...
                difference = temp;
                System.out.println(String.format("%s! Снова не угадал! Осталось %s попытки! Попробуй ещё раз!", hint, attempt));
            }
